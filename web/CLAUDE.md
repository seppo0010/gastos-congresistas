...
This file provides guidance to Claude Code (claude.ai/code) when working with code in this directory (the web app half of the repo — see the root `CLAUDE.md` for the data pipeline that feeds it).
...
npm run build     # generate:og && tsc -b && vite build && generate-person-pages (TypeScript check + bundle + static pages)
npm run lint      # ESLint
...
**Data loading**: `public/legisladores_full.json` (~4.2 MB) is imported statically at build time. No API calls at runtime. These `public/*.json` files are written by `../update.sh` (the pipeline in the repo root) and are committed to git — they're the site's actual deployed data, unlike everything else the pipeline touches.
...
**Vite config**: base path comes from `GITHUB_PAGES_BASE` env var, set by CI. Tailwind CSS 4 is used via the Vite plugin (not PostCSS).

**Build-time scripts**: `npm run build` also runs `scripts/generate-og-image.mjs` (OG image) and `scripts/generate-person-pages.mjs` (static per-person pages). These live under `web/scripts/`, not the repo-root `scripts/`, because they `import` npm packages (react/satori/resvg, vite) — Node resolves ESM imports relative to the importing file's own location, so they need to sit inside `web/` to see `web/node_modules`. They still resolve data paths via `process.cwd()` (i.e. `web/public`, `web/dist`), not `__dirname`.

**CI/CD**: `../.github/workflows/deploy.yml` builds and deploys to GitHub Pages on push to `main`. It runs `npm ci` / `npm run build` with `working-directory: web` and uploads `web/dist`.
